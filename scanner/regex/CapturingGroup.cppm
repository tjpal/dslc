...
namespace scanner {
    ...
    public:
        explicit CapturingGroup(const std::shared_ptr<RegexNode>& groupNode) : groupNode(groupNode) {}

        CapturingGroup(const std::shared_ptr<RegexNode>& groupNode, std::string name)
            : groupNode(groupNode), name(std::move(name)) {}
